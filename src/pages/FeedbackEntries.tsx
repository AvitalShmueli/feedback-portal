...
const FeedbackEntries: React.FC = () => {
  const { packageName, formId } = useParams<{
    packageName: string;
    formId: string;
  }>();
  ...
  const handleActiveToggle = async (checked: boolean) => {
    ...
    try {
      ...
      message.success(
        `Form ${checked ? "activated" : "deactivated"} successfully`
      );
    } catch (error) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setLoading(true);
      try {
        const entries = await feedbackService.getFeedbackEntries(
          packageName,
          formId
        );
  ...
  const chartData = Object.entries(statistics.rating_breakdown).map(
    ([rating, count]) => ({
  ...
  const columns: TableProps<FeedbackEntry>["columns"] = [
    ...
    },
    {
      ...
      width: 180,
      render: (device_info) =>
        device_info || <Text type="secondary">Unknown</Text>,
    },
    {
      title: "User",
      dataIndex: "user_id",
                      ...
                      <Statistic
                        ...
                        suffix={
                          <Rate
                            ...
                            allowHalf
                            value={
                              Math.round(statistics.average_rating * 2) / 2
                            }
            ...
            <div>
              {loading ? (
                ...
                <Table
                  ...
                  locale={{
                    emptyText: (
                      <Empty description="No feedback entries found" />
                    ),
                  }}
